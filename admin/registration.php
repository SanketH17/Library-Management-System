...
<section>
  <div class="reg_img">
    ...
    <?php
      ...
      if(isset($_POST['submit']))  /*To check if whether the button is pressed or not */
      {
        $count=0;
        $sql="SELECT username from `student`";
        $res=mysqli_query($db,$sql); /* connection variable and sql query variable */

        while($row=mysqli_fetch_assoc($res)) /* To check all the usernames */
        {
        ...
        if($count==0) /* If not same username already, so this username can be accepted */ 
        {
        ...
        else /*if the count value is 1 or more i.e. there are same usrnames that is the username is already exist
        so this user cannot register he has to give different username */
        {
